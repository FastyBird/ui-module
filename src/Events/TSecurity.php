...
trait TSecurity
{
	...
	public function checkSecurity(
		...
		$wsKey = $httpRequest->getHeader(UIModule\Constants::WS_HEADER_WS_KEY);
		...
		if (
			...
			$this->logger->warning('[MIDDLEWARE] Client used invalid WS key', [
				'ws_key' => $wsKey,
			]);
		...
		$origin = $httpRequest->getHeader(UIModule\Constants::WS_HEADER_ORIGIN);
		...
		if (
			...
			$this->logger->warning('[MIDDLEWARE] Client is connecting from not allowed origin', [
				'origin' => $origin,
			]);
		...
		$authToken = $httpRequest->getHeader(UIModule\Constants::WS_HEADER_AUTHORIZATION);

		if ($authToken === null) {
			$this->logger->warning('[MIDDLEWARE] Client access token is missing');
